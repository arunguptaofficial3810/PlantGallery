...
const ContactForm = () => {
    ...
    const isDisabled = value?.mobile_number?.length !== 10 || !value?.message?.length || !value?.name?.length
  ...
  const handleChange = (targetValue: string, type: string) => {
    if(type === 'name'){
        ...
        window.alert('🌸 Thank you! Your message has been received — our team will be in touch soon.')
    }
  };

  return (
    <form className={style.form} onSubmit={() => handleChange("submit","submit")}>
